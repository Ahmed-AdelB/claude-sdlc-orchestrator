...
```bash
# Standard execution with auto-approve (read-only operations)
gemini -m gemini-3-pro-preview --approval-mode yolo "$ARGUMENTS"

# With specific model (modifications require manual approval)
gemini -m gemini-3-pro-preview "$ARGUMENTS"

# For large context analysis (1M tokens)
gemini -m gemini-3-pro-preview --approval-mode yolo "Analyze entire codebase: $ARGUMENTS"
```
